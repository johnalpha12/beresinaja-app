...
export default function JualBarangPage() {
  ...
  useEffect(() => {
    async function loadContent() {
      try {
        setLoading(true)
        const response = await fetch("/api/content/jual-barang", {
          cache: "no-store",
        })

        if (!response.ok) {
          throw new Error("Gagal memuat kategori jual barang.")
        }
  ...
  if (loading) {
    return <PageLoader message="Memuat form jual barang..." />
  }
      ...
      <div className="flex-1 overflow-y-auto pb-24 px-4 lg:px-10 pt-5 flex flex-col gap-5">
        ...
        <section className="relative">
          ...
          >
            ...
            <svg
              className={`w-4 h-4 text-muted-foreground transition-transform ${categoryOpen ? "rotate-180" : ""}`}
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
