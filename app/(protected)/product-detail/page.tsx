...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    async function loadProduct() {
      try {
        const configSnap = await getDoc(doc(db, "content", "product-detail"))
        const productId = configSnap.exists()
          ? (configSnap.data() as { productId?: string }).productId
          : null
        const targetId = productId || "p1"

        const productSnap = await getDoc(doc(db, "products", targetId))
        if (productSnap.exists()) {
          setProduct(productSnap.data() as ProductDetail)
        }
      } catch (error) {
