  ...
  if (loading) {
    return (
      <div className="flex flex-col min-h-screen bg-[#F5F7FA]">
        <div className="px-4 sm:px-6 lg:px-10 pt-8 sm:pt-10 pb-6 sm:pb-8" style={{ background: "linear-gradient(135deg, #2196F3 0%, #29B6F6 100%)" }}>
      ...
      <div className="flex-1 overflow-y-auto pb-24 flex flex-col gap-4 sm:gap-5 -mt-4">
        ...
        <div className="mx-4 sm:mx-6 lg:mx-10 bg-card rounded-2xl border border-border p-4 sm:p-5">
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-4">
            {[
              { value: String(stats.services), label: "Servis" },
              { value: String(stats.transactions), label: "Transaksi" },
              { value: String(stats.sellItems), label: "Jual Barang" },
            ].map(({ value, label }) => (
        ...
        {/* Premium Card - Tampilkan hanya untuk pengguna biasa */}
        ...
        {/* Logout Button - Sekarang dengan fungsi logout yang benar */}
        <div className="mx-4 sm:mx-6 lg:mx-10">
          <button
        ...
        {/* Informasi Role untuk Teknisi (hidden di UI tapi ada di data) */}
        {userData?.role === "teknisi" && (
